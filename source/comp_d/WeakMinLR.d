...
/**********************
  Let S and S' be LR(1) states. Suppose that S and S' are weakly compatible. Let T = goto(S, Y), T' = goto(S', Y) for some symbol Y.
  Then T and T' are weakly compatible.
  
  Hence, if we let S_m be the merge of S and S', then the merge of T and T' (denoted by T_m) is equal to goto(S_m, Y).
...
package pure void closure(const GrammarInfo grammar_info, ItemGroupSet item_group_set) {
    ...
    while (true) {
        ...
        foreach (item; item_group_set.keys) {
            ...
            if (B !in grammar_info.nonterminals) continue;
...
package pure ItemGroupSet _goto(const GrammarInfo grammar_info, inout ItemGroupSet item_group_set, inout Symbol symbol) {
    ...
    foreach (item; item_group_set.keys) {
        ...
        else if (grammar_info.grammar[item.num].rhs[item.index] == symbol)
            result[LR0Item(item.num, item.index+1)] = new Set!Symbol(item_group_set[item].array);
    }
            ...
            while (!changed_states_queue.empty) {
                ...
                */
                ...
                foreach (sym; new_lookaheads.keys) {
                    ...
                    writeln("\tafter merging = {");
                    foreach (item; new_lookaheads[sym].keys) {
                        auto rule = grammar[item.num];
                        if (item.num == grammar.length-1) write("\t\t\033[1m\033[31m", item.num, "\033[0m");
                        else write("\t\t", item.num);
                        write(": [", grammar_info.nameOf(rule.lhs), "  ->  ");
