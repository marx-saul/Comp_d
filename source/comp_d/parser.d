...
unittest {
    ...
    alias parser1 = Parser!(grammar_info, "SLR", {writeln("accept");}, (x){/*reduce*/}, (x){writeln("error");});
    ...
    if ( is( typeof( { _accept(); _reduce(size_t.init); _error(State.init); _shift(); } ) ) )
{
    int parse(Range)(Range input) {
        return comp_d.parser.parse!(_accept, _reduce, _error, _shift, Range)(g_i.grammar, t_i.table, input);
    }
}
    ...
    if ( is( typeof( { _accept(); _reduce(size_t.init); _error(State.init); _shift(); } ) ) )
{
    ...
    alias Parser = generateParser!(grammar_info, table_info, _accept, _reduce, _error, _shift);
}
